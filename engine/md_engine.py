...
class Engine():
    ...
    def __init__(self, temperature=300, timestep=0.01, nsteps=500):
        ...
        # Example : Add 2 atoms
        self.system.add_atom(Atom("C", [0, 0, 0]))
        self.system.add_atom(Atom("C", [1, 0, 0]))
    ...
    def set_init_pos(self):
        for atom in self.system.atoms:
            atom.position = np.array(np.random.rand(3))

    def set_init_vel(self):
        for atom in self.system.atoms:
            # Simplified for now
            atom.velocity = np.random.normal(0, np.sqrt(self.temperature / atom.mass), 3)
